...
<?php 
$qry = $conn->query("SELECT t.*,concat(c.lastname,', ',c.firstname,' ',c.middlename) as cname, d.name as dname FROM tickets t inner join students c on c.id= t.student_id inner join departments d on d.id = t.department_id  where  t.id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
	$$k = $v;
}
?>
				...
				<div class="card-body p-0 py-2">
					<div class="container-fluid">
						...
						while($row = $comments->fetch_assoc()):
							...
							if($row['user_type'] == 3)
								$uname = $conn->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM students where id = {$row['user_id']}")->fetch_array()['name'];
						?>
